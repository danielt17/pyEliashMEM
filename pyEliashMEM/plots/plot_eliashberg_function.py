...
def plot_eliashberg_function_curve_with_constraint(eliashberg_function: pd.DataFrame, config: dict, output_folder: str) -> None:
    """
    ...
    if config["enables"]["enb_plots"]["enb_all_plots"] and config["enables"]["enb_plots"]["enb_eliashberg_function"]:
        ...
        ax.plot(eliashberg_function["omega[meV]"],
                ...
                markeredgewidth=1.5,
                color='black',
                label="Eliashberg function")
        ax.plot(eliashberg_function["omega[meV]"],
                eliashberg_function["constraint function"],
                marker="",
                linestyle="-",
                markerfacecolor='none',
                markeredgecolor='blue',
        ...
        ax.set_title(
            rf"Estimated Eliashberg function $\mathrm{{\alpha}}^{{2}} F({{\omega}})$")
        ax.set_xlabel(r"$\mathrm{{\omega}} [meV]$")
        ax.set_ylabel(r"$\mathrm{{\alpha}}^{{2}} F({{\omega}}) [meV]$")
        ax.legend()
        ax.grid(True)
        if config["enables"]["enb_saves"]["enb_all_saves"] and config["enables"]["enb_saves"]["enb_save_figures"]:
            file_name = "eliashberg_function.svg"
